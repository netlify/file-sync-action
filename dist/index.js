...
// src/lib/pr_body.js
var getPRBody = ({ localPath, remotePath, sourceRepo }) => `
...
// src/lib/repo.js
var getDefaultBranch = async ({ octokit, owner, repo }) => {
  try {
...
var getFile = async ({ octokit, owner, repo, path }) => {
  try {
...
var getSHAOfLocalFile = async (path) => {
  let file;
  try {
  ...
  const input = `blob ${file.length}\0${file}`;
  const sha1sum = crypto.createHash("sha1").update(input).digest("hex");
  return sha1sum;
};

// src/lib/files.js
var getDiff = async ({ localPath, octokit, repoAndPath }) => {
...
var processFile = async ({ branch, file, octokit, targetOwner, targetRepo }) => {
...
var getInputs = () => {
  const { FILES: envFiles, GITHUB_REPOSITORY: envRepository, GITHUB_TOKEN: envToken } = process2.env;
  const targetRepository = envRepository.split("/");
  const token = envToken || import_core.default.getInput("token");
  const rawFiles = envFiles || import_core.default.getInput("files");
  let files = {};
  try {
...
var run = async () => {
  ...
  const diff = await Promise.all(Object.entries(files).map(([repoAndPath, localPath]) => getDiff({ localPath, octokit, repoAndPath })));
  const filesToDownload = diff.filter(Boolean);
  await Promise.all(filesToDownload.map((file) => processFile({ branch: defaultBranch, file, octokit, targetOwner, targetRepo })));
};
try {
  run();
} catch (error) {
  import_core2.default.debug(`Error: ${JSON.stringify(error)}`);
